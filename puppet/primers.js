...
function postAmmoToDB(ammoList) {
  ammoList.forEach((ammo) => {
    axios.post("http://localhost:3000/ammo/add", ammo);
  });
}
...
async function parseCabelasPage(page, url) {
  ...
  for (let i = 2; i < numberOfEntries; i++) {
    ...
    let id = model + "-" + description + "-" + count + "-" + availability;

    // push entry
    primerList.push({ id, size, model, description, count, availability, url});

    // await page.screenshot({path: `./screenshots/primers/${model}-${availability}.png`});
  }
}
